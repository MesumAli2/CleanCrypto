...
import com.mesum.coindetail.components.CoinTag
import com.mesum.coindetail.components.TeamListItem
import com.mesum.domain.model.CoinDetail
import com.mesum.domain.model.TeamMember
            ...
            LazyColumn(modifier = Modifier.fillMaxSize(), contentPadding = PaddingValues(20.dp)){
              item {
                ...
                items(coin.team ?: emptyList()){teamMember: TeamMember? ->
                    TeamListItem(teamMember = teamMember ?: TeamMember(id = "", name = "", position = ""), modifier = Modifier
                        .fillMaxWidth()
                        .padding(10.dp))
                    Divider()
                }
...
fun CoinDetailScreenPreview() {
    val coin = CoinDetail(
        ...
        team = listOf(
            TeamMember(id = "", name = "John Doe", position = "Developer"),
            TeamMember(id = "", name = "Jane Doe", position = "Designer")
        ),
        coinId = "1"
    )
    ...
    Box(modifier = Modifier.fillMaxSize()) {
        state.coin?.let { coin ->
            LazyColumn(
                ...
                items(coin.team ?: emptyList()) { teamMember: TeamMember? ->
                    TeamListItem(
                        teamMember = teamMember ?: TeamMember(id = "", name = "", position = ""),
                        modifier = Modifier
                            .fillMaxWidth()
                            .padding(10.dp)
                    )
